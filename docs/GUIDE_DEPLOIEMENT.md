...
### À l'attention du responsable informatique du Collège Exemple
...
- Un **nom de domaine ou sous-domaine** pointant vers ce serveur (ex.
  `fiche-projet.exemple.fr`), avec un enregistrement DNS déjà en
  place.
...
3. **Identifiants** (menu *APIs & Services > Identifiants*) : créez un
   ...
   - **URI de redirection autorisée** à renseigner impérativement :
     ...
     `fiche-projet.exemple.fr`). Sans cette URI exacte, la
...
```
APP_BASE_URL=https://fiche-projet.exemple.fr
```
...
| `ALLOWED_EMAIL_DOMAIN` | Domaine email autorisé à se connecter (ex. `exemple.fr`). Toute autre adresse est rejetée à la connexion. |
