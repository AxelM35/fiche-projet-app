...
- ✅ **Pièces jointes (MVP : lien Drive simple)** — fait. Chaque projet a un champ `lienDrive` (URL du dossier Google Drive du voyage, migration `V5`), affiché comme bouton "Ouvrir le dossier Drive" sur la fiche projet (`formulaire.html` et `consultation.html`). Un dossier Drive par projet, créé et partagé **à la main en dehors de l'application** (ou via l'ancien script Apps Script) — l'appli ne fait qu'stocker/afficher le lien, aucune intégration API Drive, aucun scope OAuth supplémentaire. Modifiable indépendamment du statut du dossier (y compris pendant l'instruction ou une fois `VALIDE`) par l'organisateur du dossier ou par n'importe quel rôle de validation (Compta/Vie Scolaire/Direction/Admin) via `ProjetService.modifierLienDrive` — `ROLE_LECTURE_SEULE` voit le lien mais ne peut pas le modifier. URL restreinte à `drive.google.com`/`docs.google.com` (protection contre un lien arbitraire/`javascript:` stocké puis rendu cliquable).
  - ⬜ **Intégration complète** (upload/liste/suppression de fichiers directement depuis l'appli, via l'API Drive), façon prototype Apps Script — pas engagée pour l'instant, plus gros chantier (scope Drive, très probablement une revue Google avant la Production, gestion des erreurs API). Le collège est confirmé sur **Google Workspace avec accès admin possible** (compte de service + Drive partagé envisageables) si ce chantier est repris plus tard.
...
2. Pièces jointes : si on les ajoute un jour, préférence de stockage — disque du serveur, bucket compatible S3, ou Google Drive via API (vu l'écosystème Google déjà en place) ? --> Google Drive. Pour le MVP : simple lien vers un dossier Drive par projet (géré à la main), sans intégration API. Une vraie intégration (upload/liste depuis l'appli) reste possible plus tard : le collège est confirmé sur Google Workspace avec accès admin possible.
